	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{
			...
			args: args{
				...
				f: filter.Eq{}, //"id": filter.Special("AAA()")},
				o: []request.Option{
					request.WithField{"o1.id", "o.id"},
...
func TestConstructor_Select(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr1: fmt.Errorf("order: unknown column: any"),
		},
		{
			name: "",
			args: args{
		...
		},
		{
			...
			want:  `SELECT "o"."id" FROM "objects" AS "o" WHERE TRUE ORDER BY "o"."id"`,
			want1: []any{},
			want2: []any{&o.ID},
		},
		{
		...
		},
		{
			...
			want:  `SELECT "o"."id" FROM "objects" AS "o" WHERE TRUE ORDER BY RANDOM()`,
			want1: []any{},
			want2: []any{&o.ID},
		},
		{
...
func BenchmarkConstructor_Select_WithCache(b *testing.B) {
	var j model.Object
	r, err := request.NewWithOption([]request.Option{
		request.WithCreated("time_1"),
		request.WithUpdated("time_2"),
